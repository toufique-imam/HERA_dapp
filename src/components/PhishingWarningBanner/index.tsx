...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://app.defiswap or defiswap.exchange - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/app.defiswap)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://app.defiswap'}
          color={text === 'https://app.defiswap' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
