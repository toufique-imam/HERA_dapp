...
const LotteryCardContent = () => {
  ...
  const cakePrizesText = t('%cakePrizeInUsd% in DEFI prizes this round', { cakePrizeInUsd: cakePriceBusdAsString })
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      const { amountCollectedInDefi } = await fetchLottery(lotteryId)
      const prizeInBusd = cakePriceBusd.times(amountCollectedInDefi)
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="white" mb="40px">
          {t('Buy tickets with DEFI, win DEFI if your numbers match')}
        </Text>
      </Flex>
