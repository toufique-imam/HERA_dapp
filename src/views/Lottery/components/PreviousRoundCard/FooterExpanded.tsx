...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInDefi } = lotteryNodeData
    prizeInBusd = amountCollectedInDefi.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" DEFI"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInDefi)}
            decimals={0}
          />
        )}
      </>
